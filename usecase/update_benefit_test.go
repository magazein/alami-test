...
func TestEndOfDayUC_UpdateBenefit(t *testing.T) {
		...
		},
		{
			...
			fields: fields{
				afterEodRepo: func() repository.AfterEodRepoItf {
					...
					repo.EXPECT().Update(1, []string{"1", "", "", "225", "8", "", "", "", ""}).Return(nil)
					return repo
				}(),
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				afterEodRepo: func() repository.AfterEodRepoItf {
					...
					repo.EXPECT().Update(1, []string{"1", "", "", "130", "", "", "", "", "", "5", "2"}).Return(nil)
					return repo
				}(),
			},
			args: args{
				data: []string{"1", "", "", "130", "20"},
				i:    2,
			},
			wantErr: false,
		},
		...
		},
		{
			...
			fields: fields{
				afterEodRepo: func() repository.AfterEodRepoItf {
					...
					repo.EXPECT().Update(1, []string{"1", "", "", "130", "", "", "", "", "", "5", "2"}).Return(errors.New("foo"))
					return repo
				}(),
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := uc.UpdateBenefit(tt.args.i, tt.args.data); (err != nil) != tt.wantErr {
				t.Errorf("EndOfDayUC.UpdateBenefit() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
